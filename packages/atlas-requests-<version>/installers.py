#!/usr/bin/python3
"""Atlas requests Installer Helpers."""
import base64
import configparser
import hashlib
import json
...
class Config(BaseConfig):
    ...
    SQLALCHEMY_DATABASE_URI = "postgresql+psycopg2://{{user}}:{{pw}}@{{url}}/{{db}}".format(
        user="atlas_me", pw="{secrets["PG_PASS"]}", url="localhost", db="atlas_requests"
    )
"""
...
server {{
    ...
    location /static {{
        access_log   off;
        alias /usr/lib/atlas-requests/app/web/static/;
    }}

    location / {{
        access_log   off;
        include proxy_params;
        proxy_pass http://unix:/usr/lib/atlas-requests/app/gunicorn.sock;
    }}
}}
"""
...
server {{
    ...
    location / {{
        access_log   off;
        include proxy_params;
        proxy_pass http://unix:/usr/lib/atlas-requests/app/web.sock;
    }}
}}
"""
...
server {{
    ...
    location / {{
        access_log   off;
        include proxy_params;
        proxy_pass http://unix:/usr/lib/atlas-requests/app/runner.sock;
    }}
}}
...
server {{
    ...
    location / {{
        access_log   off;
        include proxy_params;
        proxy_pass http://unix:/usr/lib/atlas-requests/app/scheduler.sock;
    }}
}}
"""

    # save to config file
    Path("/etc/nginx/sites-available/atlas-requests").write_text(config)

    # link to enabled configs
    if os.path.islink("/etc/nginx/sites-enabled/atlas-requests"):
        os.unlink("/etc/nginx/sites-enabled/atlas-requests")
    os.symlink(
        "/etc/nginx/sites-available/atlas-requests", "/etc/nginx/sites-enabled/atlas-requests"
    )
