...
samlify.setSchemaValidator(validator);
  ...
  if(process.env.SAML_PRIVATE_KEY) spData.privateKey= fs.readFileSync(process.env.SAML_PRIVATE_KEY)
  if(process.env.SAML_PRIVATE_KEY_PASS) spData.privateKeyPass= process.env.SAML_PRIVATE_KEY_PASS
  if (process.env.SAML_ENC_PRIVATE_KEY) spData.encPrivateKey= fs.readFileSync(process.env.SAML_ENC_PRIVATE_KEY)
...
export async function getIdp() {
  ...
  if(process.env.SAML_PRIVATE_KEY ) idpData.privateKey = fs.readFileSync(process.env.SAML_PRIVATE_KEY)
